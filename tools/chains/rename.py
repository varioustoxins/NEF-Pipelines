...
def rename(
    ...
    use_category: bool = Option(False, '-c', '--category', help='select frames to rename chains in by category rather than name'),
    frames: List[str] = Option([], '-f', '--frame', help='limit changes to a a particular frame by name [or category if --category is set], note: wildcards are allowed, repeated uses add more frames'),
