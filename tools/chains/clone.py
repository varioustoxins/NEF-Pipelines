...
from lib.sequence_lib import chain_code_iter, sequence_from_frame, sequence_to_nef_frame

from lib.util import parse_comma_separated_options, get_pipe_file_or_exit
...
def clone(
    ...
    chain_codes: List[str] = Option(None, '-c', '--chains', help='new chain codes to add otherwise defaults to next available ascii uper case letter,'
    ...
    if len(molecular_system) > 1:
        exit_error("found more than one molecular system frame, this is not a valid nef file!")
    ...
    if target not in existing_chain_codes:
        exit_error(f"couldn't find target chain {target} in {', '.join(existing_chain_codes)}")
