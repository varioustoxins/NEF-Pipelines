...
from lib.util import exit_error, cached_stdin,  running_in_pycharm, parse_comma_separated_options
...
def insert(
    ...
    use_categories: bool = typer.Option(False, '-c', '--category', ),
    select: List[str] = typer.Option(None, '-s', '--select',help=SELECT_HELP ),
    priority: PRIORITY = typer.Option(PRIORITY.FILE, '-p', '--priority', help=f'source of frames to prioritise, one of {PRIORITY.STDIN} and {PRIORITY.FILE}'),
    nef_file_paths: List[Path] = typer.Argument(..., help="nef files from which to insert frames into the current stream")
):
