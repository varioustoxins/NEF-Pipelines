...
from lib.util import read_integer_or_exit, exit_error
from pynmrstar import Entry, Saveframe
from lib.typer_utils import get_args
from argparse import Namespace
from tabulate import tabulate as tabulate_formatter, tabulate_formats
...
FRAMES_HELP = 'selectors for loops to tabulate. note: wild cards are allowed and specific loops can be chosen (see above)'
...
def tabulate(
        ...
        format: str = typer.Option('plain', '-f', '--format', help=FORMAT_HELP, metavar='format'),
        verbose: bool = typer.Option(False, '-v', '--verbose', help='include frame names etc in output'),
        full: bool = typer.Option(False, '--full', help="don't suppress empty columns [default: False]"),
        no_abbreviations: bool =  typer.Option(False, '--no-abbreviations', help="dont' abbreviate column headings"),
        frame_selectors: List[str] = typer.Argument(None, help=FRAMES_HELP, metavar='<loop-selector>...'),
):
    """- tabulate loops from frames in a NEF file.     notes: 1. using the name of a frame will tabulate all loops, using
         frame.loop_index [e.g. moleculecular_system.1] will tabulate a specific loop. 2. wild cards can be use for frame
         names e.g. mol would select molecular_system" and anything other frame whose name contains mol 3. by default
...
def _select_loop_indices(frames):
    ...
    for frame_selector in frames:
        ...
        if len(frame_selector_index) == 1:
            ...
            exit_error(f'[frames tabulate] too many fields int the frame selector {".".join(frame_selector)}')
    for frame_selector, indices in frame_indices.items():
        if ALL_LOOPS in indices and len(indices) > 1:
            ...
            message = f'[frames tabulate] incompatible selections, you selected all loops and a specific index for frame "*{frame_selector}*", indices were: {index_strings}'
            exit_error(message)
...
def _should_output_loop(index, frame_name, frame_indices):
    ...
    for frame_selector in frame_indices:
        if fnmatch.fnmatch(frame_name, f'*{frame_selector}*'):
            ...
            if index not in indices and not ALL_LOOPS in indices:
                do_output = False
    return do_output
