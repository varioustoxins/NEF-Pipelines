...
target_frames ={'nef_chemical_shift_list' : ['_nef_chemical_shift'], 'nef_nmr_spectrum': ['_nef_peak']}

def offset_chemical_shifts(entry, args):

    if not args.target:
        exit_error('WARNING: i need a target nucleus or atom name provided by -t / --target,  such as *CB* or 13C')
...
def read_args():
    ...
    parser.add_argument('-o', '--offset', metavar='OFFSET', type=float, default=0.0, dest='offset',
                        help='offset to add to shifts')
    parser.add_argument('-t', '--target', metavar='TARGET', type=str, default=None, dest='target',
                        help='nucleus or atom to offset')
    parser.add_argument('-s', '--spectra', metavar='OFFSET', type=str, default='*', dest='spectra',
                        action='append', nargs='+', help='nucleus  or atom to offset')
