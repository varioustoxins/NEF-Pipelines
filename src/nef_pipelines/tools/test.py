...
try:
    ...
    def test(
        ...
        show_capture: bool = typer.Option(
            ...
            "-s",
            "--show-capture",
            help="show print statements and captured output",
        ),
    ...
    def _find_pytest_commands(root_path, targets):
        ...
        if not pytest_root:
            exit_error(
                f"couldn't find pytest root directory starting from: {root_path}"
            )
    ...
    print(
        f"ERROR: there was an exception {e} during test startup, exiting...",
        file=sys.stderr,
    )
