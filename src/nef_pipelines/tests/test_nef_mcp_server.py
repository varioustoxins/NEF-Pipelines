"""
Tests for nef_mcp_server_integrated.py - MCP server tool functions.
"""
...
from nef_mcp_server_integrated import (
    nef_execute_command,
...
EXPECTED_HELP_SECTIONS = ["Usage:", "Options", "Housekeeping"]
...
def test_nef_get_readme():
    """\
    Test nef_get_readme returns complete readme with all expected sections.
    """
    result = nef_get_readme()

    # Verify response structure
    assert isinstance(result, dict)
    assert result["success"] is True
    assert isinstance(result["readme_content"], str)
...
def test_nef_get_skill_file():
    """\
    Test nef_get_skill_file attempts to find skill file.
    """
    result = nef_get_skill_file()

    # Skill file may or may not exist, just check structure
    assert isinstance(result, dict)
    ...
    if result["success"]:
        ...
        # Should provide error message when not found
...
def test_nef_execute_command_returns_dict_structure():
    ...
    assert isinstance(result, dict)
    assert set(result.keys()) == EXPECTED_FIELDS, f"Fields mismatch. Got: {set(result.keys())}"
...
def test_nef_execute_pipeline_returns_dict_structure():
    ...
    """
    EXPECTED_FIELDS = {"stdout", "stderr", "exit_code", "success", "step_results", "failed_step"}
