...
from lib.chem_comp.object_iter import ObjectIter, return_entity
...
class _Mutator:
    ...
    def process_items(self, target):
        return self._process_items([target, ])[0]
...
class _IDSAsStrings:
    ...
    def __call__(self, path, target):
        ...
        if isinstance(target, list):
            for index, item in enumerate(target):
                if isinstance(item, str) and item.startswith('_') and item[1:].isdigit():
    ...
    def __call__(self, path, target):
        ...
        for container, type_or_types in self._container_type_pairs:
            if isinstance(target, dict) and container in target:
                ...
                types = [type_or_types, ] if not isinstance(type_or_types, (list, tuple)) else type_or_types

                for _type in types:

                    if isinstance(target[container], dict) and _type in target[container]:
...
def load_xml_chem_comp_as_json(file_name, verbose=False):
    ...
    APPLICATION_DATA_TYPES = {('applicationData', 'AppDataInt'), ('applicationData', 'AppDataString'),
                              ('applicationData', 'AppDataBoolean')}

    add_discriminators = _Mutator(_AddDisciminatorsToApplicationData(APPLICATION_DATA_TYPES))
    chemcomp = add_discriminators.process_items(chemcomp)
    ...
    if 'CHEM.NonStdChemComp' in chemcomp_data:
        raise ValueError(f'Error: Non standard chem comps are not currently supported {file_name}')

    return ChemComp(**chemcomp_data)

def convert_xml_chem_comps(chem_comp_paths: List[Path], out_dir:Path=None, force=False):
...
def type_children(target, parent):
    ...
    if hasattr(target, '__origin__'):
        ...
        result = tuple([ObjectIter.Entry(target, i, elem) for i, elem in enumerate(target)])

    elif isinstance(target, dict):
        result = tuple([ObjectIter.Entry(target, key, value) for key, value in target.items()])
            ...
            if file_path.suffix == '.xml':
                ...
                try:
                    ...
                    if json_chem_comp.exists() and not force:
                        exit_error(f'file {json_chem_comp} exists to overwite use then force option')
                    ...
                    caught=e
                    if not 'Error: Non standard chem comps are not currently supported' in caught.args[0]:
...
if __name__ == '__main__':
    ...
    json_chem_comp_paths = find_chem_comps()
