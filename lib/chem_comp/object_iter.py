...
def return_path_and_value(iter, item):
...
def object_children(target, parent):
    ...
    if hasattr(target, '__fields__'):
        result = tuple(
            [ObjectIter.Entry(target, field_name, getattr(target, field_name)) for field_name in target.__fields__.keys()]
        )
...
class ObjectIter:
    ...
    def __init__(self, child_lister=object_children, result_generator=return_path_and_value):
    ...
    def iter(self, target):
        ...
        while nonvisited:
            ...
            children = [elem for elem in self._child_lister(item.value, item) if not elem.is_leaf]
