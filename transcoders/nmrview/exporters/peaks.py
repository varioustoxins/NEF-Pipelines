...
from transcoders.nmrview import export_app
import typer

from lib.nef_lib import create_entry_from_stdin_or_exit, loop_row_dict_iter, loop_row_namespace_iter

from lib.util import parse_comma_separated_options,  exit_error, is_float, is_int
...
def _peak_to_atom_labels(row: Namespace) -> List[AtomLabel]:
    ...
    for i in range(1, 15):
        ...
        if dim_chain_code in row and dim_sequence_code in row and dim_residue_name in row and dim_atom_name in row:
...
def _convert_to_floats_or_exit(putative_floats: List[str], thing_types: str) -> List[float]:
    ...
    if not all(are_floats):
        bad_dims = [dim for dim, is_float_value in enumerate(are_floats, start=1) if not is_float_value]
        bad_values = [value for is_float_value, value in zip(are_floats, putative_floats)
...
def _row_to_peak(axis_names: Tuple[str], row: Dict[str, Union[int,str, float]]) -> Peak:
    peak_id = row['peak_id']
    volume = row['volume'] if row['volume'] != NEF_UNKNOWN else None
    volume_uncertainty = row['volume_uncertainty'] if  row['volume_uncertainty'] != NEF_UNKNOWN else None
    height = row['height'] if row['height'] != NEF_UNKNOWN else None
    height_uncertainty = row['height_uncertainty'] if row['height_uncertainty'] != NEF_UNKNOWN else None
    ...
    for axis_index, axis_name in enumerate(axis_names, start=1):
        ...
        position_uncertainties[axis_name] = row[position_uncertainty_tag] if row[position_uncertainty_tag] != NEF_UNKNOWN else None
    ...
    uncertainties_none = [uncertainty is None for uncertainty in position_uncertainties.values()]
    if all(uncertainties_none):
        position_uncertainties = None

    peak_values = PeakValues(serial=peak_id+1, volume=volume, height = height, volume_uncertainty=volume_uncertainty, height_uncertainty=height_uncertainty)
    return Peak(id=peak_id, values=peak_values, positions=positions, position_uncertainties=position_uncertainties, assignments=assignments)
...
def _peak_to_assignments(peak: Peak, axis_names: Tuple[str]) -> List[str]:
    ...
    if max_num_assignments == 0:
        ...
        result = {}
        for axis_name in axis_names:
            result[axis_name] = ' '.join([_peak_to_nmrview_label(atom_label) for atom_label in peak.assignments[axis_name]])
...
@export_app.command()
def peaks(
    file_name_template: str = typer.Option('%s', help="the template for the filename to export to %s will get replaced by the axis_name of the peak frame", metavar='<peak-file.xpk>'),
    frame_selectors: List[str] = typer.Argument(None, help='the names of the frames to export', metavar='<frames>')
    ...
    names_and_frames = {frame.name[len(SPECTRUM_CATEGORY):].lstrip('_') : frame  for frame in peaks}

    selected_frames = {}
    for frame_selector in frame_selectors:
        selection = {name: frame for name, frame in names_and_frames.items() if fnmatch(name, frame_selector)}

        selected_frames.update(selection)

    if len(selected_frames) == 0:
        for frame_selector in frame_selectors:
            selection = {name: frame for name, frame in names_and_frames.items() if fnmatch(name, f'*{frame_selector}*')}

            selected_frames.update(selection)

    if not '%s' and len(selected_frames) > 1:
        exit_error(f'%s is not in the filename template and there is more than one file, template {file_name_template}')
    ...
    for frame_name, frame in selected_frames.items():
        ...
        axis_names = [dimension.axis_code if 'axis_code' in dimension and dimension != NEF_UNKNOWN else 'unknown'
        ...
        spectrometer_frequencies = [dimension.spectrometer_frequency for dimension in spectrum_dimensions]
        spectrometer_frequencies = _convert_to_floats_or_exit(spectrometer_frequencies, 'spectrometer_frequencies')
        ...
        sweep_widths =  [sweep_width* spectrometer_frequency for sweep_width, spectrometer_frequency in
        ...
        spectrometer_frequencies = [f'{{{spectrometer_frequency:.3f}}}' for spectrometer_frequency in spectrometer_frequencies]
        result.append(' '.join(spectrometer_frequencies))

        print('\n'.join(result))

        headings = [f'{axis_name}.{item}' for axis_name in axis_names for item in 'LPWBEJU']
