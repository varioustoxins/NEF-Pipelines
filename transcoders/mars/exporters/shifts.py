...
from lib.util import chunks, exit_error, read_float_or_exit, is_int, is_float
...
@export_app.command()
def shifts(
    shift_frames: List[str] = typer.Option([], '-f', '--frame', help='selector for the shift restraint frames to use, can be called multiple times and include wild cards'),
    ...
    for shift in shifts:
        ...
        # first deal with completely unassigned residues chain_code @- and sequence_code @xxxx where xxx is the pseudo residue
        ...
        if chain_code.startswith('@-') and  sequence_code.startswith('@'):
            ...
            negative_offset = sequence_code_fields[1] if len(sequence_code_fields) == 2 else 0
            ...
            pseudo_atom = PseudoAtom(sequence_code, shift.atom.residue_name, negative_offset, atom_name=atom_name)
