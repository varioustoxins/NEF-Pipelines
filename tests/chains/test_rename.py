...
def test_rename_select_frame_by_name(typer_app, using_chains, monkeypatch, clear_cache):
    ...
    result = runner.invoke(typer_app, [*RENAME_CHAINS_A_E, '--frame', 'test'], input=open(path))
...
# noinspection PyUnusedLocal
def test_rename_select_frame_by_category(typer_app, using_chains, monkeypatch, clear_cache):
    monkeypatch.setattr('sys.stdin.isatty', lambda: False)
