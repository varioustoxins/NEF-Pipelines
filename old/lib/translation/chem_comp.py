...
from lib.translation.object_iter import (
    ObjectIter,
    item_only,
    object_children,
    return_entity,
...
class ID(int):
    ...
    def validate(cls, v):
        ...
        try:
            ...
            raise ValueError(
                f"invalid ID format [should look like an integer: {v} error was: {e}],"
            )
...
class ChemAtomSet(PCBaseModel):
    ...
    isProchiral: Attribute[
        Optional[bool]
    ]  # 1..1  # no default value bu not always present
    name: Attribute[str]  # 1..1
    subType: Attribute[int] = 1  # 1..1
...
class LinkEnd(PCBaseModel):
    ...
    boundChemAtom: Link[
        OptionalItem[ChemAtom]
    ]  # 1..1 is required but not always present?
    boundLinkAtom: Link[
        OptionalItem[LinkAtom]
    ]  # 1..1 is required but not always present?
    chemComp: Parent[ChemComp]  # 1..1
    remoteChemAtom: Link[
        OptionalItem[ChemAtom]
...
def get_annotations(
    o: object, include_super_classes: bool = True
    ...
    def _make_links(self):
        ...
        for j, item in enumerate(for_children):
            ...
            if isinstance(value, ID):
                ...
                setattr(
                    item.target, item.selector, self.__class__._object_map[item.value]
                )
